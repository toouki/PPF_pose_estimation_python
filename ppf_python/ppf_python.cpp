...
PYBIND11_MODULE(ppf, m) {
    ...
    py::class_<ppf::Detector>(m, "Detector")
        ...
        .def("matchScene", [](ppf::Detector &self, const ppf::PointCloud &scene,
                               float samplingDistanceRel = 0.04f, float keyPointFraction = 0.2f,
                               float minScore = 0.2f, ppf::MatchParam param = ppf::MatchParam(),
                               ppf::MatchResult *matchResult = nullptr) {
            // 创建输出向量
            std::vector<Eigen::Matrix4f> poses;
            std::vector<float> scores;
            
            // 调用matchScene方法
            self.matchScene(scene, poses, scores, samplingDistanceRel,
                           keyPointFraction, minScore, param, matchResult);
            
            // 返回结果
            return py::make_tuple(poses, scores);
        },
             py::arg("scene"),
             py::arg("samplingDistanceRel") = 0.04f,
